...
RSpec.describe 'Api::V1::Auth', type: :request do
  describe 'POST /api/v1/auth/register' do
    ...
    context 'with valid parameters' do
      it 'creates a new user and returns token' do
        ...
        expect_json_response(['id', 'username', 'token'])
  ...
  describe 'POST /api/v1/auth/login' do
    ...
    context 'with non-existent user' do
      it 'returns unauthorized error' do
        post '/api/v1/auth/login', params: { username: 'nonexistent', password: 'password123' }, headers: default_headers
  ...
  describe 'GET /api/v1/auth/current_user' do
    ...
    context 'with valid token' do
      it 'returns current user information' do
        ...
        expect(response).to have_http_status(:ok)
        expect_json_response(['id', 'username', 'profile'])
